...
@app.post("/create/", response_model=Contact)
...
@app.get("/get/{contact_id}", response_model=Contact)
...
@app.get("/list/", response_model=List[Contact])
def read_contacts(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    contacts = db.query(Contact).offset(skip).limit(limit).all()
    return contacts


@app.put("/update/{contact_id}", response_model=Contact)
